...
def setup_config_pc(serial_number,UUID,Motherboard):

    resultado_existe_maquina = acao_comum_banco(f"SELECT idMaquina,fkEscola FROM safeClass.maquina WHERE serialNumberBios = '{serial_number}' and uuid = '{UUID}' and serialNumberMotherboard = '{Motherboard}';")
    if resultado_existe_maquina == []:
        Fkescola = config_escola()
        limpar_tela()
        nome_maquina = input("Nomeie a Máquina Cadastrada: ")
        acao_comum_banco(f"INSERT INTO safeClass.maquina (idMaquina, uuid,serialNumberBios,serialNumberMotherboard,fkEscola,nomeIndentificacao) VALUES (default,'{UUID}','{serial_number}','{Motherboard}','{Fkescola}','{nome_maquina}');")
        FkMaquina = acao_comum_banco(f"SELECT idMaquina FROM safeClass.maquina WHERE serialNumberBios = '{serial_number}' and uuid = '{UUID}' and serialNumberMotherboard = '{Motherboard}';")
...
def config_escola():
    ...
    inep_val = acao_comum_banco(f"SELECT nomeDaEscola,idEscola FROM safeClass.escola WHERE codigoInep = '{codigo_inep_input}';")
    print(inep_val)
    while inep_val == []:
        limpar_tela()
        print("-----------------------------------------------------")
    ...
    limpar_tela()
    print("-----------------------------------------------------")
    print(f"Deseja cadastrar um computador na {inep_val[0][0]}")
    operacao = input("Continuar (s / n) ").lower()
    while operacao != 's' and operacao != 'n':
        limpar_tela()
        print("-----------------------------------------------------")
        print(f"Deseja cadastrar um computador na escola {inep_val[0][0]}")
        operacao = input("Continuar (s / n)").lower()
    if operacao == "n":
        config_escola()
    ...
    while True:
        ...
        if tentavivas < 3:
            ...
            codigo_acesso_input = input("Coloque o código de configuração da sua organização: ")
        codigo_vali = acao_comum_banco(f"SELECT fkEscola FROM safeClass.codigoConfiguracao WHERE chaveConfiguracao = '{codigo_acesso_input}' and status = 'Ativo' and fkEscola = '{inep_val[0][1]}';")
        if codigo_vali != []:
            break
...
def configurar_componetes_a_monitorar(FkMaquina,FkEscola):
    ...
    if acao == 's':
        if monitorar_cpu_percent == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},1,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_cpu_freq == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},2,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_memoria_percent == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},3,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_memoria_total == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},4,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_disco_uso == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},5,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_disco_livre == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},6,{FkEscola});"
            acao_comum_banco(query)
        if monitorar_disco_total == "s":
            query = f"INSERT INTO safeClass.maquinaMonitoramento (fkMaquina,fkComponentesAMonitorar,fkEscola) VALUES ({FkMaquina},7,{FkEscola});"
            acao_comum_banco(query)
        limpar_tela()
    else:
        configurar_componetes_a_monitorar(FkMaquina,FkEscola)
...
# Faz o comando pelo o powershell
...
setup_config_pc(serial_number,UUID,Motherboard)
Fkescola = acao_comum_banco(f"SELECT fkescola FROM safeClass.maquina WHERE serialNumberBios = '{serial_number}' AND uuid = '{UUID}' AND serialNumberMotherboard = '{Motherboard}';")
FkMaquina = acao_comum_banco(f"SELECT idMaquina FROM safeClass.maquina WHERE serialNumberBios = '{serial_number}' AND uuid = '{UUID}' AND serialNumberMotherboard = '{Motherboard}';")
Fkescola = Fkescola[0][0]
FkMaquina = FkMaquina[0][0]
FkComponetes = acao_comum_banco(f"SELECT fkComponentesAMonitorar FROM safeClass.maquinaMonitoramento WHERE fkEscola = {Fkescola} AND fkMaquina = {FkMaquina};")
limpar_tela()
...
def captura():
    query = "INSERT INTO safeClass.leitura (fkEscola,fkMaquina,fkComponentesAMonitorar,medida) VALUES"
